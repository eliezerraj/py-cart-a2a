...
from infrastructure.adapter.handler import handler_cart_price_analysis, handler_cluster_fit, handler_cluster_data
...
class A2ARouter:
    ...
    def route(self, registry, envelope):
        with tracer.start_as_current_span("a2a.router.route") as span:
            ...
            try:
                if envelope.message_type == "CART_PRICE_ANALYSIS":
                    return handler_cart_price_analysis(registry, envelope.payload)
                elif envelope.message_type == "CART_CLUSTER_FIT":
                    return handler_cluster_fit(registry, envelope.payload)
                elif envelope.message_type == "CART_CLUSTER_DATA":
                    return handler_cluster_data(registry, envelope.payload)
                else:
